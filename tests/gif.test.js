...
test("Default optimize", async () => {
	const buf = await readFileAsync(
		path.join(__dirname, "test_images", "avocado.gif"),
	);
...
test("resize width to 300 px", async () => {
	...
	const diff = compare(
		await getPixelsAsync(Buffer.from(data)),
		await getPixelsAsync(
			await readFileAsync(
				path.join(__dirname, "test_images", "avocado_300.gif"),
			),
		),
	);
...
test("crop image", async () => {
	...
	const diff = compare(
		...
		await getPixelsAsync(
			await readFileAsync(
				path.join(__dirname, "test_images", "avocado_crop.gif"),
			),
		),
	);
...
test("flip horizontally", async () => {
	...
	const diff = compare(
		...
		await getPixelsAsync(
			await readFileAsync(
				path.join(__dirname, "test_images", "avocado_flip_h.gif"),
			),
		),
	);
...
test("flip vertically", async () => {
	...
	const diff = compare(
		...
		await getPixelsAsync(
			await readFileAsync(
				path.join(__dirname, "test_images", "avocado_flip_v.gif"),
			),
		),
	);
...
test("Interlaced output", async () => {
	...
	const diff = compare(
		...
		await getPixelsAsync(
			await readFileAsync(
				path.join(__dirname, "test_images", "avocado_interlaced.gif"),
			),
		),
	);
...
test("throws error when wrong parameter passed", async () => {
	...
	).rejects.toThrow(/expects a nonnegative integer/);
});
